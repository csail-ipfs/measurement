...
def scrape(hashlist, output, timeout=30):
	...
	for _, h in tqdm(hashes):
		try:
			result = requests.get(API.format(h), timeout=timeout)
...
if __name__ == '__main__':
	...
	if len(sys.argv) != 4:
		print("Usage: python3 {0} <hashlist.txt> <timeout> <output.json>".format(sys.argv[0]))

	results, timeouts = scrape(HASH_LIST, OUTPUT, TIMEOUT):
	with open(OUTPUT, 'w') as f:
		json.dump( {'timeouts': timeouts, 'results': results}, f)
