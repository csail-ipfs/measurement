...
if not config['ip']:
    raise ValueError(
        "Please update the `config` variable before running."
    )
...
def pingMyOldPeers(daysAgo):
    ...
    print(
        "Obtaining peers from {d} days ago on {vp}...".format(
            d=daysAgo, vp=VP
        )
    )
    with open(
        os.path.join(
            STORAGE,
            "ping.{vp}.time.config".format(
                vp=VP, time=daysAgo * 86400
            ),
        ),
        'w',
    ) as f:
...
def pingSubsetList(fn="peersubset.txt"):
    ...
    for peer in tqdm(peers):
        with open(
            os.path.join(
                STORAGE, "ping.{host}".format(host=VP)
            ),
            "a",
        ) as out:
            try:
...
def getOldKPs(vantage, daysAgo):
    ...
    query = {
        'VANTAGE': vantage,
        'TIMESTAMP': {
            '$gte': now - seconds - window,
            '$lte': now - seconds + window,
        },
    }
...
def ping(addr, timeout=6):
    ...
    output = list(
        jsonlines.Reader(response.text.split("\n")[0:-1])
    )
    ...
    if len(output) == 1:
        ...
        raise Exception(
            "IPFS API did not return valid value."
        )
